...
type CONFIGS\openssl.SIGNCERT.cnf >> CONFIGS\openssl_local
...
if "%SHA%" equ "" echo Usage:  make.signcert.cmd ^<certificate name^> ^<CA Cert Name^> [[sha1 or sha256] [rsa1024 or rsa2048 or rsa4096]] && goto end
...
if "%RSA%" equ "" echo Usage:  make.signcert.cmd ^<certificate name^> ^<CA Cert Name^> [[sha1 or sha256] [rsa2048 or rsa1024 or rsa4096]] && goto end

openssl x509 -req -%SHA% -days 1024 -CA CA\%2.cer -CAkey CA\%2.key -in %1 -CAserial PROCESS\%2\serial -extfile CONFIGS\openssl_local -extensions v3_ca -out %1.crt
...
if "%if_pkcs12%" equ "y" openssl pkcs12 -export -in %1.crt -inkey %1.key -out %1.p12

echo.
echo.
echo Complete.
if "%if_pkcs12%" equ "y" echo %1.crt, %1.key, and %1.p12 have been created && goto end
echo %1.cer and %1.key have been created
goto end

:INFO
echo ####### Certificate Signer #####
echo #
echo # Description: Signs a certificate request with a specified CA certificate. Customization of the web server certificate is provided
echo #     in the CONFIGS\openssl.WEBSERVER.cnf file in the [ v3_ca ] section.
echo #
echo # Usage:  make.signcert.cmd ^<certificate name^> ^<CA Cert Name^> [[sha1 or sha256] [rsa2048 or rsa1024 or rsa4096]]  
echo #
echo #    Ex. make.signcert.cmd c:\webserver1.alpha.com.csr ca.alpha.com
echo #
echo #######
goto end
